...
public:
   ...
   */
  void AddTXChar(uint8_t *TxChar, uint8_t len)
  {
    uint8_t _TxLen = len, i;

    if (_TxLen > FILTERSIZE) _TxLen = FILTERSIZE;
 
    // save UUID filter (reversed)
    for (int c = _TxLen-1, i = 0 ; c > -1 ; c--, i++) _TxCharUUID.u[i] = TxChar[c];
    
    // save length of UUID received ( 2, 4 or 16 bytes )
    _TxCharUUID.ulen = _TxLen;
    _TxCharUUID.handle = 0;       // handle not found yet
  }

 /* write to command from central to peripheral. 
  * return true if OK, else if error
  */
  bool WriteTXChar(uint8_t *data, uint8_t len)
  {
    if (_TxCharUUID.handle != 0) {
 ...
 * called after disconnect, else you get memory dump
 */
...
void when_service_discovered(const DiscoveredService *discovered_service)
{
  ...
  // although you can add a UUID filter during discovery Mbed call.. it is only ONE service
...
void when_characteristic_discovered(const DiscoveredCharacteristic *discovered_characteristic)
{
  ...
  if (_TxCharUUID.ulen > 0 && _TxCharUUID.handle == 0){
    ...
    if ( s_uuid.getLen() == _TxCharUUID.ulen ) {
       
      // check on UUID match
      for (uint8_t i = 0; i < _TxCharUUID.ulen;  i++) {
        // printf("UUIID %x, value %x\n", _UUIDs[j].u[i], uuid_value[i]);
        if (_TxCharUUID.u[i] != uuid_value[i]) match = false;
      }
      
      // we have a match
      if (match) {
        _TxCharUUID.handle = discovered_characteristic->getValueHandle();
#ifdef GattClientInfo
...
void when_descriptor_written(const GattWriteCallbackParams* event)
{
  ...
  else if (_TxCharUUID.handle == event->handle) {

#ifdef GattClientInfo
    printf("TX written at %u.\r\n", _TxCharUUID.handle);
#endif
