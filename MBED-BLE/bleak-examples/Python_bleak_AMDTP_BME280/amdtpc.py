...
The AMD Transfer Protocol enables a exchanging larg datapackages between Peripheral and central. It does this by breaking that data packet
into smaller packages. The size is based on the agreed MTU size. Once all the packages have been received it will perform a CRC checked on the
...
AMDTP_STATUS_RECEIVE_DONE =          0x0a
AMDTP_STATUS_MAX =                   0x0b
...
AMDTP_CONTROL_REQ_READY =    0x01     # request a SEND-READY
AMDTP_CONTROL_SEND_READY =   0x02
...
class AmdtpClient():
    ...
    def AmdtpPacketHandler(self):
        ...
        if self.rxtype == AMDTP_PKT_TYPE_DATA:
            ...
            """
                ...
                package
            """
            control = self.rxPckt[0]
            resendPktSn = self.rxPckt[1]
            ...
            if control == AMDTP_CONTROL_SEND_READY:
                ...
                if resendPktSn > self.lastRxPktSn:
                    AmdtpSendReply(AMDTP_STATUS_RESEND_REPLY)

                elif resendPktSn == self.lastRxPktSn:
                    AmdtpSendReply(AMDTP_STATUS_SUCCESS)
    ...
    def AmdtpReceivePkt(self, data, len):
        ...
        if self.rxOffset == 0:
            ...
            self.ack_enabled = (data[2] & extractor) >> 7
            #print (bin(data[2]))

            extractor = int("01000000", 2)
            self.encrypt_enabled = (data[2] & extractor) >> 6
            ...
            if self.rxtype == AMDTP_PKT_TYPE_DATA:
                ...
                if self.NextRxPktSn != self.rxPktsn:
                    ...
                    """
                        There is no recovery here. The sender (peripheral / server) has already overwritten the TX buffer with the new data
                        Hence it is only displayed as a warning.
                    """
        ...
        print("sending before ", self.rxtype, self.ack_enabled )
        ...
        if self.rxtype == AMDTP_PKT_TYPE_DATA and self.ack_enabled:
            ...
            print("sending control", self.NewChunkReceived)
            AmdtpSendControl(AMDTP_CONTROL_SEND_READY, self.NewChunkReceived, 1)
    ...
    def AmdtpBuildPkt(self, ptype, encrypted, enableACK, buf, len):
        ...
        if (encrypted):
            header1 = (1 << 8)                                                  # set encryption bit (not used yet)

        if self.txLen > self.attMtuSize - 3:                                    # length of data + header (4) + CRC (4)
            header1 = header1 | (1 << 7)                                        # set control acknowledgement bit
    ...
    def AmdtpSendPacketHandler(self):
        ...
        if  self.txState == AMDTP_STATE_SENDING:
            ...
            if self.AMD_debug:
                printf("Could not send txState is not IDLE")
