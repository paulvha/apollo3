...
//*************************** ADDED SPECIALS FOR SPEED READING ***********************
// using 2 different in case someone wants to perform BOTH single and multiple (CRAZY !!)
static bool ap3_adc_initialized = false;    // flag to show if the ADC has been initialized
static bool ap3_adc_initialized_M = false;  // needed in analogReadeFast() multiple

static bool ap3_adc_Pin_initialized = false;// needed in analogReadSingleFast()
static PinName ap3_adc_fast_pin;            // needed in analogReadSingleFast()

am_hal_adc_slot_config_t ADCSlotConfig;     // needed to configure slots
bool analogReadTurbo_initialized = false;   // needed in analogReadTurbo()

struct FastInit {                           // needed in analogReadFast()
    PinName pinName;                        // pinnumber
    bool    Pin_initialized;                // if NOT initialized is false
};

# define MAXPINS 5                          // keep track of how many pins
FastInit InitPins[MAXPINS];                 // keep track of Pins initialized
am_hal_adc_sample_t Sample[MAXPINS];        // store sample results

/** special cutdown version of the original analogRead() to increase the reading speed
 *
 * @param  pinName the ADCname  (like A0 or A29)
 * @param  ReInit if true performs another reinitialize of the pin
 *
 * This is the V1 version: it will on read 1 ADC pin, only using ADC slot 0
 * If you use the same pin all the time, it will initialize only once the slot
 * and thus you can achieve 146Khz sample rate instead of 11Khz on AnalogRead()
 *
 * If you use a different pin each read, the sample rate will be about 11Khz
 */
uint16_t analogReadSingleFast(PinName pinName, bool ReInit)
{
    // initialize the ADC module if needed
    if (!ap3_adc_initialized)
    {
        initializeADC();
        ap3_adc_initialized = true;
        ap3_adc_initialized_M = false;
        ap3_adc_Pin_initialized = false;
    }
    else {
        // if new pin
        if (ap3_adc_fast_pin != pinName)
            ap3_adc_Pin_initialized = false;
    }

    // initialize the pin
    if (!ap3_adc_Pin_initialized || ReInit)
    {
        // run ONCE
        // get offset in table
        ...
        // if no config pointer : trouble !
        if(!config){ return 0; }

        ap3_adc_fast_pin = pinName;     // save pinName for check later
        ap3_adc_Pin_initialized = true;

        // perform setting and reading if first time or forced
        return ap3_analog_read(config);
    }

    // if pinName was used before and no reinit, perform the reading here
 ...
 * special cutdown version of the original analogRead() to increase the reading speed
 *
 * @param  pinName the ADC name (like A0 or A29)
 * @param  ReInit if true performs another SLOT-reinitialize of the pin
 *
 * This is V2. it allows for MAXPINS to be handled in parallel. Each pinNumber is assigned
 * to its own ADC SLOT and as such 100Khz samples rate can be achieved, compared to 11Khz
 * for a standard analogRead()
 */
uint16_t analogReadFast(PinName pinName, bool ReInit)
{
    uint8_t offset;

    // initialize the ADC module ONCE
    if (!ap3_adc_initialized_M)
    {
        initializeADC();
        ap3_adc_initialized_M = true;
        ap3_adc_initialized = false;

        // from ap3_change_channel. Applies to each slot
        ADCSlotConfig.eMeasToAvg = AM_HAL_ADC_SLOT_AVG_1;
        ADCSlotConfig.ePrecisionMode = AM_HAL_ADC_SLOT_14BIT;
        ADCSlotConfig.bWindowCompare = false;
        ADCSlotConfig.bEnabled = true;

        // initialize the structure to keep track of pin
        for (offset = 0; offset < MAXPINS; offset++) {
            InitPins[offset].pinName = NC;
            InitPins[offset].Pin_initialized = false;
        }
    }

    // check is new pin or earlier used pin
    for (offset = 0; offset < MAXPINS; offset++) {

        // if match
        if (InitPins[offset].pinName == pinName) {
            break;
        }

        // empty slot
        if (InitPins[offset].pinName == NC) {
            InitPins[offset].pinName = pinName;
            break;
        }
    }
    ...
    if (offset == MAXPINS) {
        Serial.println("Error - No empty slot");
        return(0);
    }

    // initialize the pin (needed first time or forced)
    if (InitPins[offset].Pin_initialized == false || ReInit)
    {
        // run ONCE (hopefully)
        // get offset in table
        pin_size_t index = pinIndexByName(pinName);
        ...
        // which to pad to use as input
        ADCSlotConfig.eChannel = config->eChannel;

        InitPins[offset].Pin_initialized = true;
        ...
        // select different ADC slot based on offset
        if (AM_HAL_STATUS_SUCCESS != am_hal_adc_configure_slot(g_ADCHandle, offset, &ADCSlotConfig))
        {
            Serial.println("Error - configuring ADC Slot failed.\n");
            return AP3_ERR;
        }
    ...
    uint32_t ui32IntMask;
    uint32_t ui32NumSamples = MAXPINS;          // max samples (or less if not MAXPINS)
    ...
    MBED_ASSERT(AM_HAL_STATUS_SUCCESS == am_hal_adc_samples_read(g_ADCHandle, false, NULL, &ui32NumSamples, &Sample[0]));

    uint32_t result = Sample[offset].ui32Sample;

    // Shift result depending on resolution
    return (_analogReadResolution > AP3_ADC_RESOLUTION) ? (result << (_analogReadResolution - AP3_ADC_RESOLUTION)) : (result >> (AP3_ADC_RESOLUTION - _analogReadResolution));
}

/** Special version to increase the reading speed.
 *
 ...
 * @param  ReInit if true performs another reinitialize of the pins
 *
 * Perform initialization in the sketch setup().
 * On first call each pin is allocated to a different ADC slot.
 * This first call will take ~700us to also init ADC module.
 *
 * With each subsequent call will it reads ALL the enabled ADC slots and return the ADC values in the structure .
 * This routine will peform only ONCE a read and then return all the read values.
 *
 * Each subsequent read-cycle / analogReadTurbo()-call takes about 11us or 90Khz. During a single read-cycle it can sample
 * up to 5 (MAXPINS) different pins as a theorie it could be  5 x 90Khz = 450Khz
 *
 * @return  the pinnumbers stored
 *
 */
uint16_t analogReadTurbo(ADC_Turbo *res, uint8_t NumPin, bool ReInit)
{
    uint8_t i;

    // if not initialized yet
    if (!analogReadTurbo_initialized)
    {
        // set for each pin allocate a slot
        for (i = 0; i < NumPin ;i++){
            analogReadFast(res[i].pinName, ReInit);
            res[i].adc = 0x0;
        }

        analogReadTurbo_initialized = true;
    }
    else {
        // perform a single read, BUT all pins requested are read
        analogReadFast(res[0].pinName);
    }

    // Obtain the result for each ADC pin.
    for (i = 0; i < NumPin ;i++) {

        uint32_t result = Sample[i].ui32Sample;

        // Shift result depending on resolution

        res[i].adc = (_analogReadResolution > AP3_ADC_RESOLUTION) ? (result << (_analogReadResolution - AP3_ADC_RESOLUTION)) : (result >> (AP3_ADC_RESOLUTION - _analogReadResolution));
    }
...
int ap3_analog_read(ap3_adc_channel_config_t* config){
    ...
    // BEGIN special : check initialized paulvha
    size_t idx;
    bool is_init = false;
    for( idx = 0; idx < (sizeof(ap3_adc_inits)/sizeof(ap3_adc_initialized)); idx++){
        if(ap3_adc_inits[idx].pinNumber == config->pinNumber){
            is_init = ap3_adc_inits[idx].initalized;
            ap3_adc_inits[idx].initalized = true;
            break;
        }
    }

    if (!is_init) {     // special : perform init ONCE paulvha
        // configure the pin as an input if not an internal channel
        if(config->internal_channel == AP3_ADC_INTERNAL_CHANNELS_NUM){
            uint8_t funcsel = config->funcsel;
            am_hal_gpio_pincfg_t pincfg = g_AM_HAL_GPIO_INPUT;
