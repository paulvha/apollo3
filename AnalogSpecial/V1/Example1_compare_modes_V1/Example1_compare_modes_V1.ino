 ...
 *
 * During a standard analogRead() implementation only ADC SLOT-0 is used. With each analogRead() there are
 * large number of checks, followed by a setup / re-init of SLOT-0 everytime as only this SLOT is used.
 * All these checks are valid and necessary to prevent unexpected behaviour.
 *
 * In analogReadTurbo() & analogReadFast() & analogReadSingleFast() many of the checks are removed and expects the usage is only
 * to read the ADC value on one of the 10 ADC pins.
 * Not all pins are exposed external on all Apollo3 boards. See the board pinout information on Sparkfun.com.
 *
 * analogReadTurbo() and analogReadFast() are mend to be used to read multiple ADC pins in a fast manner
 * An Apollo3 has 8 ADC slots that can be used. In analogReadTurbo() and analogReadFast() setup each pin will
 * get its own ADC-Slot assigned and enabled. With each read-cyle all the enabled slots are read in one go.
 * Although maximum 5 ADC-pins are possible (set by default in the adjusted driver), this example is using 3 pins
 *
 ...
 * A first call to AnalogReadTurbo() or analogReadFast() will also initialize the Apollo3 module + ADC-slot.
 ...
 * ** Reading ADCPin1, ADCPin2 and ADCPin3 : In this case of reading 3000 samples, 1000 * 3 pin results, each sample takes avg. 3.7us or ~269KHz
 *
 ...
 * ** Reading a single ADCPin1 : In this case of reading 3000 samples, 3000 * ADCPin1 results, each sample takes avg. 9.13 us per sample or 109KHz sampling rate.
 *
 ...
 * Not exactly given some extra overhead time to copy results and counting in the loop.
 *
 * background on analogReadTurbo() multi pins read:
 * The read-cycle time for multiple pins (11.21us / 89Khz) is close to the time for a read-cycle for one pin (9.13us / 109KHz)
 * But in this example we read 3 pins in a single read-cycle, the sample frequency per pin is 3 x 89Khz = 267Khz. Imagine if you need to use all 5....
 * 
 *----------------------------------------------------------------------------------------------------------------------
 * analogReadFast()
 * 
 ...
 * ** Reading ADCPin1, ADCPin2 and ADCPin3 : In this case of reading 3000 samples, 1000 (Read_cycles) * 3 pin results, each sample takes avg. 8.9us or ~112KHz.
 *
 * Setting Read_mode as 6 : With analogReadFast() only ADCpin1 result read are returned to be processed / displayed later.
 * ** Reading a single ADCPin1 : In this case of reading 3000 samples, 3000 * ADCPin1 results, each sample takes avg. 7.54us per sample or ~132KHz sampling rate.
 *
 * Each ADCpin is obtained in a separate call, however all the enabled pins are read by the Apollo3 processor. If you have 3 different pins, they are all3 enabled
 ...
 * ** Reading ADCPin1,then ADCPin2 and then ADCPin3 :In that case 3000 samples, 1000 (Read_cycles) * 3 pin results, each sample takes avg. 86.27us per sample or 12KHz sampling rate.
 *
 * Setting Read_mode as 7 : With analogReadSingleFast() only ADCpin1 result read are returned to be processed / displayed later.
 * ** Reading a single ADCPin1 : In this case of reading 3000 samples, 3000 * ADCPin1 results, each sample takes avg. 6.85 us per sample or 146KHz sampling rate.
 *
 ...
 * ** Reading ADCPin1,then ADCPin2 and then ADCPin3 : In that case 3000 samples, 1000 (Read_cycles) * 3 pin results, each sample takes avg. 78us or 12.8Khz.
 *
 * Setting Read_mode as 8 : The standard analogRead() only ADCpin1 result read are returned to be processed / displayed later.
 * ** Reading a single ADCPin1 : In this case of reading 3000 samples, 3000 * ADCPin1 results, each sample takes avg. 78us or 12.8Khz.
 ...
 * Only 1 ADCpin  (Single Fast) 146Khz, (Fast) 132Khz, (Turbo)      109Khz or (Standard) 12Khz
 * 3 ADCPins      (Turbo)       269Khz, (Fast) 112Khz, (Single Fast) 12Khz or (Standard) 12Khz
 *
 *----------------------------------------------------------------------------------------------------------------------
 * struct ADC_Turbo  {
 *   uint8_t pinNumber;
 ...
 * ------------------------------------------------------------------------------------
 *
 * @param  pinNumber the ADCnumber (like A0 or A29)
 * @param  ReInit if true performs another reinitialize of the ADC Slot for the pin
 * @return The ADC value read.
 *
 * uint16_t analogReadFast(uint8_t pinNumber, bool ReInit = false);
 * uint16_t analogReadSingleFast(uint8_t pinNumber, bool ReInit = false);
 *
 */
...
// define number of ReadCycles. 
// The total number of samples taken = NumPin * ReadCycles
...
void setup() {
   ...
   Serial.begin(115200);
   Serial.println("\nExample1_V1: Compare Analog Read Speed Test");
