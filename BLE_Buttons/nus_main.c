...
extern void Handle_button(uint8_t b);  //// SPECIAL_BUTTON ADDED (see BLE_example_funcs.cpp line 310)
...
//// SPECIAL_BUTTON START
#define ATT_UUID_BUTTONS      0x183B    /*!< \brief buttons */
...
static const uint8_t tagScanData[] =
{
  ...
  UINT16_TO_BYTES(ATT_UUID_BUTTONS),
  0
};
//// SPECIAL_BUTTON END
...
//// SPECIAL_BUTTON ADDED START
/* call back for BUTTON write back
 * so when something is written to this device
 */
uint8_t
buttons_write_cback(dmConnId_t connId, uint16_t handle, uint8_t operation,
                   uint16_t offset, uint16_t len, uint8_t *pValue, attsAttr_t *pAttr)
{
    ...
    if (handle == BUTTONS_RX_HDL)
    {
        Handle_button(pValue[0]);
    }
    

  return(0);
}
//// SPECIAL_BUTTON ADDED END
/*************************************************************************************************/
/*!
 *  \fn     NusStart
 *
...
void NusStart(void)
{
  ...
  //AppUiBtnRegister(tagBtnCback);      //// SPECIAL_BUTTON removed not needed
  ...
  SvcCoreAddGroup();
  //SvcPxCbackRegister(NULL, tagIasWriteCback);   //// SPECIAL_BUTTON removed not needed
  //SvcPxAddGroup();  //// SPECIAL_BUTTON removed not needed

  // add buttons
  SvcButtonsCbackRegister(NULL, buttons_write_cback);//// SPECIAL_BUTTON ADDED
  SvcButtonsAddGroup();   //// SPECIAL_BUTTON ADDED
