 ...
 *
 * A BME280 is connected with I2C to the ATP. The ATP is connected with SPI to a controller.
 *
 ...
 *
 * A BME280 is connected to the peripheral ATP with I2C. There is a menu (press <enter> to show)
 ...
 * peripheral / HOST communicating on SPI
  ===============================================================================
    ...
    *1) the handshake pin is only necessary when using FIFO to exchange data
...
void setup() {
  ...
  SERIAL_PORT.print("Example2 IOS peripheral with SPI ");
  ...
  // start IOS in SPI mode
  IOS.beginSPI(UseFiFo);
...
void UpdateStatusReg()
{
  ...
  if (UseFiFo && NewData) {
    ...
    if (! IOS.set_interrupt_host(AM_HAL_IOS_IOINTCTL_INT0)) {
     SERIAL_PORT.printf(F("Error during sending interrupt host\n"));
    }
  }
 ...
 * This also enables that FIFO is selected in the library
 *
 */
