...
// called from attrib_callbacks_remove to remove the request from
// the queue and free the buffer
...
// called after g_attrib_send, NOT when it is completed successfull
...
/** called back after sending has completed from g_attrib_send and downstream
 * final call from handle_rsp(), which is called from can_read_data() after sending was complete
 */
                    ...
                    uint16_t length, void *user_data)
{
    ...
    if (cb->result_func)    // = call back function ato user level
