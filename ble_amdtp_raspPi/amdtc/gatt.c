                            ...
                            gpointer user_data)
{
    ...
    // not all the entries/handles have been received
    if (end != 0xffff) {
        size_t buflen;
...
// discover primary service. if UUID is provided it will be selective, otherwise all
                            ...
                            gpointer user_data)
{
    ...
    // if provided search selective else look for all from the device
    if (uuid) {
        dp->uuid = *uuid;
