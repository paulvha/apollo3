...
// Artemis / Apollo3
// December 2021 / Paulvha
#elif defined (ARDUINO_ARCH_APOLLO3)

static inline __attribute__((always_inline))
uint8_t directRead(uint8_t pin)
{
    uint32_t ui32PinState;
    am_hal_gpio_state_read(pin, AM_HAL_GPIO_INPUT_READ, &ui32PinState);

    return((uint8_t) ui32PinState);
}
...
#define IO_REG_MASK_ATTR
#define DIRECT_READ(base, pin)          directRead(pin)
#define DIRECT_WRITE_LOW(base, pin)     am_hal_gpio_state_write(pin, AM_HAL_GPIO_OUTPUT_CLEAR)
#define DIRECT_WRITE_HIGH(base, pin)    am_hal_gpio_state_write(pin, AM_HAL_GPIO_OUTPUT_SET)
#define DIRECT_MODE_INPUT(base, pin)    am_hal_gpio_pinconfig(pin, g_AM_HAL_GPIO_INPUT)
#define DIRECT_MODE_OUTPUT(base, pin)   am_hal_gpio_pinconfig(pin, g_AM_HAL_GPIO_OUTPUT)
//#warning "OneWire for Artemis & Apollo3 V1"
