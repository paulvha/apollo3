...
// you can select scan with callback or detect in loop
// some boards will crash with callback due to the amount of traffic
// see scan.txt
...
void setup() {
  Serial.begin(115200);
  while (!Serial);
  Serial.println("BLE example 22 iBeacon scan");
...
bool CheckBeacon(BLEDevice peripheral) {
  ...
  // should never happen  2 x 31 is max!!
  if (dlen > 65) {
