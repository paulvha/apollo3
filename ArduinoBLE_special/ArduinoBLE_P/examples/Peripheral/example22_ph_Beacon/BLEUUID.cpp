...
BLEUUID::BLEUUID(String value) {
  ...
  if (value.length() == 4) {
    ...
    char val[16];
    value.toCharArray( val,16);
    memrcpy(m_uuid.uuid.uuid128, (uint8_t*)val, 16);
  }
    ...
    Serial.println("ERROR: UUID value not 2, 4, 16 or 36 bytes");
    m_valueSet = false;
  }
} //BLEUUID(String)
...
BLEUUID::BLEUUID(uint8_t* pData, size_t size, bool msbFirst) {
  if (size != 16) {
    Serial.println("ERROR: UUID length not 16 bytes");
    return;
  }
  m_uuid.len = ESP_UUID_LEN_128;
  if (msbFirst) {
 ...
 */
BLEUUID::BLEUUID(bt_uuid_t uuid) {
  m_uuid     = uuid;
  m_valueSet = true;
} // BLEUUID
...
uint8_t BLEUUID::bitSize() {
  ...
  switch (m_uuid.len) {
    ...
    case ESP_UUID_LEN_128:
      return 128;
    default:
      Serial.print("Unknown UUID length: ");
      Serial.println(m_uuid.len);
      return 0;
  } // End of switch
} // bitSize
...
BLEUUID BLEUUID::fromString(String _uuid) {
  ...
  if(len == 4) {
    uint16_t x = strtoul(_uuid.substring(start, len).c_str(), NULL, 16);
    return BLEUUID(x);
  } else if (len == 8) {
    uint32_t x = strtoul(_uuid.substring(start, len).c_str(), NULL, 16);
    return BLEUUID(x);
  } else if (len == 36) {
