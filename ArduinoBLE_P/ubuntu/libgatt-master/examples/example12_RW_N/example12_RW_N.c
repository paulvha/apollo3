 ...
 ****************************************************************************
 *
 * This example12_RW_N started based on read_write.c example, but that ended
 * up to be only 5% of the RW_N_example.
 *
 ...
 * as the example12_ph_RW_Notify.ino.
 *
 ...
 *
 * When you request a new message, it will be send in blocks of MAXBLOCKSIZE length.
...
char *default_dev_name = "Example12 BLE";
...
size_t RecBlockLength = MAXBLOCKSIZE;   //
/////////////////////////
...
static void usage(char *argv[]) {
  ...
  fprintf(stderr, "-l  set receive blocklength. (default = %ld)\n", RecBlockLength);
...
void SendCommand(uint8_t cmd)
{
  ...
  if (gattlib_write_char_by_uuid(connection, &w_uuid, s, 2) != 0) {
...
void ReadDataBlock()
{
  ...
  // read Block
  size_t len = RecBlockLength;

  // if no memory allocated
  if (!BlockBuf) {

    BlockBuf = (uint8_t *) malloc(len);
  ...
  if (TotalMessageLength == 0) {
    ...
    // if previous message buffer pending
    if (MessageBuf) free(MessageBuf);
...
int main(int argc, char *argv[]) {
  ...
  /* parse commandline */
    while ((opt = getopt(argc, argv, "a:b:d:t:shf:l:")) != -1)
    {
    switch (opt) {
      case 'a':   // adapter
      ...
      case 'l':   // provide different blocklength than default
        RecBlockLength = (size_t) strtod(optarg, NULL);
        break;
