...
bool ATTClass::connect(uint8_t peerBdaddrType, uint8_t peerBdaddr[6])
{
    ...
    // increase timeout
    /*
     * LE_Scan_Interval,    0x0060   0x0004 to 0x4000  * 0.625 ms
    LE_Scan_Window,         0x0030   0x0004 to 0x4000 BUT less than scan interval * 0.625 ms
    Initiator_Filter_Policy, 0x00     white list ??
    Peer_Address_Type,
    Peer_Address,
    Own_Address_Type,       0x00    public
    Connection_Interval_Min,   6  *1.25ms  =   7.5ms  0x0006 to 0x0C80
    Connection_Interval_Max,   c = 12 x 1.25 = 15ms   0x0006 to 0x0C80 (larger then interval min)
    Connection_Latency,        0  0x0000 to 0x01F3
    Supervision_Timeout,       0x000A to 0x0C80 * 10ms
    Min_CE_Length,   4         0x0000 to 0xFFFF * 0.625 ms
    Max_CE_Length    6         0x0000 to 0xFFFF * 0.625 ms
* */
  if (HCI.leCreateConn(0x0060, 0x0030, 0x00, peerBdaddrType, peerBdaddr, 0x00,
  //                      0x0006, 0x000c, 0x0000, 0x00c8, 0x0004, 0x0006) != 0) {
                        0x0006, 0x000c, 0x0000, 0x00D0, 0x0004, 0x0006) != 0) {
    return false;
  }
...
// sent notification to all connected clients
...
void ATTClass::handleNotifyOrInd(uint16_t connectionHandle, uint8_t opcode, uint8_t dlen, uint8_t data[])
{
  ...
  //uint8_t handle = handleNotifyOrInd->handle;     //  wrong paulvha
  ...
  while (reqEndHandle == 0xffff) {
    ...
    if (responseBuffer[0] == ATT_OP_READ_BY_GROUP_RESP) {
      ...
      for (int i = 2; i < respLength; i += lengthPerService) {
        ...
        // either NO filter and match to the UUID filter
        if (serviceUuidFilter == NULL ||
...
bool ATTClass::discoverCharacteristics(uint16_t connectionHandle, BLERemoteDevice* device)
{
  ...
  uint8_t responseBuffer[_maxMtu];
    ...
    // for each Service now find the Characteristics
  int serviceCount = device->serviceCount();
