...
// if true we are running as peripheral {paulvha}
    ...
    if (_recvBuffer[0] == HCI_ACLDATA_PKT) {
      if (_recvIndex > 5 && _recvIndex >= (5 + (_recvBuffer[3] + (_recvBuffer[4] << 8)))) {
        ...
        digitalWrite(NINA_RTS, HIGH);
          ...
          _recvIndex = 0;
          // skip HCI_ACLDATA_PKT header before passing on
...
// reset is already done as part of begin
int HCIClass::reset()
{
...
int HCIClass::sendAclPkt(uint16_t handle, uint8_t cid, uint8_t plen, void* data)
{
  ...
  struct __attribute__ ((packed)) HCIACLHdr {
    ...
    uint16_t plen;
    uint16_t cid;
  } aclHdr = { HCI_ACLDATA_PKT, handle, uint8_t(plen + 4), plen, cid }; // 4 or 9 ?????????
...
void HCIClass::handleAclDataPkt(uint8_t /*plen*/, uint8_t pdata[])
{
  ...
  if (aclHdr->cid == ATT_CID) {                    // 0x4

    if (aclFlags == 0x01) {
      // use buffered packet (skip accel headers
  ...
  else if (aclHdr->cid == SIGNALING_CID) {      // 0x05
...
void HCIClass::handleEventPkt(uint8_t /*plen*/, uint8_t pdata[])
{
  ...
  if (eventHdr->evt == EVT_DISCONN_COMPLETE) {          // 05 ???
    struct __attribute__ ((packed)) DisconnComplete {
      uint8_t status;
      uint16_t handle;
      uint8_t reason;       // 08 means connection timeout
    ...
    // {paulvha} : Add this check else it will cause  a crash is NOT peripheral.
    if (ScanResponseDataSet) HCI.leSetAdvertiseEnable(0x01);

  } else if (eventHdr->evt == EVT_CMD_COMPLETE) {       //0x0e
  ...
  } else if (eventHdr->evt == EVT_CMD_STATUS) {         //0x0f
  ...
  } else if (eventHdr->evt == EVT_NUM_COMP_PKTS) {      //0x13
...
// also ATTClass::connect
        ...
        } else if (leMetaHeader->subevent == EVT_LE_ADVERTISING_REPORT) {           //0x02
